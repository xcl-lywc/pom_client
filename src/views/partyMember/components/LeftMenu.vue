<template>
    ...
    :unique-opened="true"
    @select="navClicked">
    <div v-for="(itemO,indexO) in navJson.partyMember">
...
<script>
  ...
  export default{
    ...
    methods: {
      navClicked(val) {   
        if(val.includes('PMTypeArticleList')){
          this.$router.push({name: val.split('/')[0], params: {key_type: val.split('/')[1]}});
        }else{
