  ...
  <div v-if="articleInfo"> 
    ...
    <el-card> 
      <div class="article-box">
        ...
        <p class="mart20">关键词：<span class="text-dim"><el-tag v-for="(item, index) in articleInfo.keyWords" :key="index" class="marr10">{{item}}</el-tag></span></p>
    ...
    <el-card class="padb20 mart20">
      ...
      <el-card class="marb20" shadow="never" v-for="(item, index) in commentList.rows">
        ...
        <p class="text-right text-dim">
          <el-avatar shape="square" class="marr5 comment-photo" :size="30" :src="common.imgPath + item.picture"> 
    ...
    <el-card class="mart20">
      <p class="text-dim">
        <el-avatar shape="square" class="marr5 comment-photo" :size="30" :src="common.imgPath + userBaseInfo.head_img"> 
          <img src="https://cube.elemecdn.com/e/fd/0fc7d20532fdaf769a25683617711png.png"/>
        </el-avatar>
  ...
  export default {
    ...
    data() {
      return {   
        ...
        typeName:     sessionStorage.getItem('article-type-name'),
    ...
    methods: {
       ...
       */
      getQueryDetailById(){
        this.axios.get(`${this.common.basePath}/${this.requestUrlType}/${this.articleId}/detail?partyOrgId=${this.orgId}&userId=${this.accessKey}`, '').then((response) => {
       ...
       */
      queryCommentList(){ 
        this.axios.post(`${this.common.basePath}/${this.requestUrlType}/review/list`, this.reviewSearchForm).then((response) => {
      ...
      like(){
        this.axios.get(`${this.common.basePath}/${this.requestUrlType}/like?id=${this.articleId}&userId=${this.accessKey}`, '').then((response) => {
          this.getQueryDetailById();
      ...
      learn(){
        this.axios.get(`${this.common.basePath}/${this.requestUrlType}/learn/${this.articleId}?userId=${this.accessKey}`, '').then((response) => {
          this.getQueryDetailById();
          this.$message.success(response.meta.message);
        }).catch((errorMsg) => { 
          this.$message.error(errorMsg)
      ...
      collect(){
        this.axios.post(`${this.common.basePath}/${this.requestUrlType}/collect?id=${this.articleId}&userId=${this.accessKey}`, '').then((response) => {
          this.getQueryDetailById();
          this.$message.success(response.meta.message);
        }).catch((errorMsg) => { 
          this.$message.error(errorMsg)
       ...
       */
      submitReview(){   
        this.axios.post(`${this.common.basePath}/${this.requestUrlType}/review`, this.commentForm).then((response) => {
          this.queryCommentList();
          this.$message.success(response.meta.message);
        }).catch((errorMsg) => { 
          this.$message.error(errorMsg)
