    ...
    <div v-if="fileList && fileList.length > 0"> 
      <el-tag
        ...
        <div style="display:inline-block;" :style="isDownload ? 'cursor: pointer' : ''"  v-if="
          ...
          <img :src="`${common.filePath}${tag.path}`" :onerror="errimage" class="thumbnail fl"/>
...
<script>
  export default {
    ...
    methods: {
      ...
      handleClick(val){
        // console.log(val)
        val.id === undefined ? this.$message.error('无法下载！！') : this.downloadFile(val.id);
      },

      /**
       * 文件流的下载
       * @author chuanlin.Xiao
       * @date   2020-01-09T15:50:23+0800
      ...
      downloadFile(fileId){
        ...
        let config = {
          ...
          method: "post",
          url: `${this.common.basePath}/file/download?fileId=${fileId}`,
          data: {}
        }
        ...
        // get
        // this.axios.get(`${this.common.basePath}/file/download`,{responseType: "blob",params:{fileId:fileId}}).then((response) => {
        //   this.common._fileBold(response);
        //   this.$message.success('下载成功')
        // }).catch((errorMsg) => {
        //   this.$message.error(errorMsg)
        ...
        // this.axios.get(`${this.common.basePath}/file`,{params:{fileId:fileId}}).then((response) => {
        //   // this.common._fileBold(response);
        //   // this.$message.success('下载成功')
        // }).catch((errorMsg) => {
        //   this.$message.error(errorMsg)
