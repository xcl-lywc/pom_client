  ...
  <div>
    ...
    <el-card class="padb20">
      ...
      <div class="article-box">
        <p class="mart10" v-for="(item, index) in dataList.rows"> 
          <el-link class="title" :underline="false" 
            @click="$router.push({name: articleDetailName, params: {type_id: typeId, article_id: item.id}})">
             {{item.articleName}}
          </el-link> 
          <span class="date">{{common._convertDate(item.createTime, 'yyyy-MM-dd')}}</span>
        </p> 
      </div>
      <el-pagination   
         class="fr mart20"
...
<script> 
  export default {
    ...
    data() {
      return { 
        typeId:           this.$route.params.type_id ? this.$route.params.type_id: this.typeComId,
        ...
        searchForm: {
          ...
          special:         '',
          partyOrgId:      this.$route.params.type_id ? this.$route.params.type_id: this.typeComId,
        },
    ...
    computed: {
      articleDetailName(){ 
        return `${this.$route.name.split('ArticleList')[0]}ArticleDetail`
      }
    },
    watch: {
    },
    ...
    methods: {
       ...
       */
      getQueryList(){   
        this.axios.post(`${this.common.basePath}/${this.typeKey}/article/list/ed`, this._queryListParams()).then((response) => {
